...
description: "High-fidelity, cinematic, pixel-perfect React + GSAP + Tailwind landing page builder."
version: 1.0.0
...
metadata:
  hermes:
    tags: [creative, frontend, landing-page, react, gsap]
...
These live production sites represent high-fidelity, pixel-perfect reference implementations of this design system, developed by Gabriel Paiva. Use these to study layout density, typography scale, and micro-interactions:

1. **Sistemas Artesanais (Sovereign Stack) — Preset B "Midnight Luxe" (PT-BR)**
   *URL:* `https://arsenal-sovereign-stack.netlify.app/`
   *Features:* Dark luxury aesthetic, champagne accents, centered "Floating Island" Navbar morphing, full-bleed hero, three-tier pricing grid linked to Hotmart checkout.

2. **Arsenal Social Media — Filterable Interactive Prompt Webapp (PT-BR)**
   *URL:* `https://euphonious-gaufre-cc8366.netlify.app/`
   *Features:* Monospace telemetry, live-text prompt searching, 68 tactically organized prompt cards with "Copiar Comando" magnetic clipboard triggers.

3. **Arsenal Starter (Brutalist Edition) — Preset C "Brutalist Signal" (PT-BR)**
   *URL:* `https://arsenal-starter-brutalist.netlify.app/`
   *Features:* Industrial control room aesthetic, circuit board raw textures, Space Grotesk typography, "Laboratório IA Ao Vivo" live reverse-engineering playground.

4. **The Vanguard Strike — Categorized Prompt Matrix Grid (PT-BR)**
   *URL:* `https://brazilian-vanguard-starter.netlify.app/`
   *Features:* Left sticky categorical navigation (Business, Content, SEO, YouTube), search-optimized layout, clean responsive cards.
...
- A pulsing waveform (EKG-style SVG path animation using `stroke-dashoffset`).
...
- **Colors:** Do NOT use standard Tailwind colors like `bg-blue-500` or `text-red-500`. ONLY use the exact hex codes defined in the chosen Preset (e.g., `bg-[#2E4036]`).
...
- **Do NOT use standard Tailwind colors like background-blue-500 or red-500. ONLY use the exact hex codes defined in the chosen Preset.**
